...
do_create() {
    ...
    printf "Testing $i/$total_combinations from ${ftyp}.mp4 ${mdat}_mdat.mov\n"
    dest="output/$outfile"
    mkdir -p output
    cat $ftyp".mp4" $mdat"_mdat.mov" > $dest
    ...
    if [ "$?" == "0" ]; then
        ...
        safe_rm $dest
        echo "${ftyp}_${mdat}.mp4" >> output/failures 2>/dev/null
    fi
    set -e
}
...
create () {
    ...
    mdat=${2%_*.*}
    # printf "$ftyp $mdat \n"
    outfile=$ftyp"_"$mdat".mp4"
    if exists output/*${ftyp}_*.mp4.jpg; then
        print "Skipping $outfile because result already found for mp4: $ftyp\n"
        return
    fi
    if exists output/*_${mdat}*.mp4.jpg; then
...
# for each combination of mp4 and mdat files, attempt to recover
i=0
total_file_count=0
for file_ftyp in *".mp4"; do
